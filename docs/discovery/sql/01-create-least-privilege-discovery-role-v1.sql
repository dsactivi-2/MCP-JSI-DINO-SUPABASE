...
-- This file is a production mutation and requires its own approved gate.
-- The password is requested interactively by psql and never belongs in Git.
...
\password dino_crm_discovery_ro_v1

SET LOCAL ROLE dino_crm_discovery_ro_v1;
    ...
    AND (
      ...
      OR (
        ...
        AND pg_catalog.has_table_privilege(
          current_user, relation_entry.oid, 'UPDATE'
        )
      )
    )
  ...
  WHERE relation_entry.relkind IN ('r', 'p', 'v', 'm', 'f')
    ...
    AND (
      pg_catalog.has_column_privilege(
        current_user, relation_entry.oid, attribute_entry.attnum, 'INSERT'
      )
      OR (
        relation_entry.oid <> 'pg_catalog.pg_settings'::pg_catalog.regclass
        AND pg_catalog.has_column_privilege(
          current_user, relation_entry.oid, attribute_entry.attnum, 'UPDATE'
        )
      )
    )
...
SELECT 1 / (
  ((current_user = 'dino_crm_discovery_ro_v1'
...
\echo PASS: least-privilege discovery role created
