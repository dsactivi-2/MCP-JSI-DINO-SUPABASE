...
readonly GATE_ID='DISCOVERY-GATE-B3-V3-RO-2026-09-12'
readonly TARGET_ALIAS='dino_crm_discovery_ro_v1'
readonly REPO_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd -P)"
readonly LAUNCHER_PATH="${REPO_ROOT}/scripts/discovery/run-gate-b3-v3-ro.sh"
readonly PROD_SQL_PATH="${REPO_ROOT}/docs/discovery/sql/20-definitions-statistics-gate-b3-v3.sql"
readonly PROD_SQL_SHA256='f537a21f1293dbe9ee614f80cc3339266d02660aabc14b3ad71d99d5b69d9a75'
...
readonly PROD_APPROVAL_ATTEST="${PROD_CONFIG_ROOT}/${TARGET_ALIAS}.b3.approval"
...
readonly STREAM_GUARD="${REPO_ROOT}/scripts/discovery/gate_b3_stream_guard.py"
readonly STREAM_GUARD_SHA256='4cd16d7873b9bed64e45b62229cc15cb5c0af99ca8bf74a394863d1e1554d770'
...
main() {
  ...
  GATE_B3_EXPECTED_DATABASE_NAME="${expected_database}" \
  ...
  PGPASSFILE="${credential_file}" \
    "${PYTHON_BIN}" "${STREAM_GUARD}" \
      --psql "${psql_bin}" \
      --sql "${sql_path}" \
      --raw "${raw_dir}/gate-b3.out" \
      --manifest "${raw_dir}/gate-b3-manifest.json" \
      --gate-id "${GATE_ID}" \
