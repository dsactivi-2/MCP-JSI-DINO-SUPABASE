...
readonly GATE_ID='DISCOVERY-GATE-B2-V3-RO-2026-09-12'
readonly TARGET_ALIAS='dino_crm_discovery_ro_v1'
readonly REPO_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd -P)"
readonly LAUNCHER_PATH="${REPO_ROOT}/scripts/discovery/run-gate-b2-v3-ro.sh"
readonly PROD_SQL_PATH="${REPO_ROOT}/docs/discovery/sql/10-catalog-structure-gate-b2-v3.sql"
readonly PROD_SQL_SHA256='6c841d716ea1895796ded317a7d90e3009c17b26095dd70f3ed6aa543e7cff08'
...
readonly PROD_APPROVAL_ATTEST="${PROD_CONFIG_ROOT}/${TARGET_ALIAS}.b2.approval"
...
readonly STREAM_GUARD="${REPO_ROOT}/scripts/discovery/gate_b2_stream_guard.py"
readonly STREAM_GUARD_SHA256='984ec441c8e901cbe113fd2e53351dbf45cb620fcafdc1d80a585d3e5637374c'
...
main() {
  ...
  GATE_B2_EXPECTED_DATABASE_NAME="${expected_database}" \
  ...
  PGPASSFILE="${credential_file}" \
    "${PYTHON_BIN}" "${STREAM_GUARD}" \
      --psql "${psql_bin}" \
      --sql "${sql_path}" \
      --raw "${raw_dir}/gate-b2.out" \
      --manifest "${raw_dir}/gate-b2-manifest.json" \
      --gate-id "${GATE_ID}" \
